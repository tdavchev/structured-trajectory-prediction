...
class InfoVAE(object):
  def __init__(self, z_size=32, x_dim=3, batch_size=1, learning_rate=0.0001, is_training=False, reuse=False, gpu_mode=False):
    self.z_size = z_size
    self.x_dim = x_dim
    ...
    with tf.variable_scope('conv_vae', reuse=self.reuse):
      if not gpu_mode:
        with tf.device('/cpu:0'):
          tf.logging.info('VAE: Model using cpu.')
          self._build_graph()
      else:
  ...
  def _build_graph(self):
    ...
    with self.g.as_default():
      ...
      self.x = tf.placeholder(tf.float32, shape=[None, 64, 64, self.x_dim])
      ...
      if self.is_training:
        ...
        self.mmd_loss = self.compute_mmd(self.epsilon, self.z)

        self.loss = self.r_loss + self.mmd_loss
